...
type ReportCardStorageRecord = {
  ...
  term: string
  session: string
  subjects: ReportCardStorageSubject[]
...
const REPORT_CARD_STORAGE_KEY = "reportCards"

const readStoredReportCards = (): ReportCardStorageRecord[] => {
  try {
    const raw = safeStorage.getItem(REPORT_CARD_STORAGE_KEY)
    if (!raw) {
      return []
    }
    ...
    return parsed.filter((entry): entry is ReportCardStorageRecord => {
      if (!entry || typeof entry !== "object") {
        return false
      }

      const candidate = entry as Partial<ReportCardStorageRecord>
      return (
    ...
    logger.error("Failed to parse stored report cards", { error })
    return []
  }
}

const writeStoredReportCards = (records: ReportCardStorageRecord[]) => {
  safeStorage.setItem(REPORT_CARD_STORAGE_KEY, JSON.stringify(records))
}

const buildReportCardStorageKey = (studentId: string, className: string, term: string, session: string) =>
  `${studentId}::${className}::${term}::${session}`

const mergeSubjectEntries = (
  subjects: ReportCardStorageSubject[],
  incoming: ReportCardStorageSubject,
  referenceName: string,
) => {
  const normalized = referenceName.trim().toLowerCase()
  const filtered = subjects.filter((subject) => subject.name.trim().toLowerCase() !== normalized)

  return [...filtered, incoming].sort((a, b) => a.name.localeCompare(b.name))
}
...
export function TeacherDashboard({ teacher }: TeacherDashboardProps) {
  ...
  const persistReportCardDrafts = useCallback(() => {
    ...
    const existingRecords = readStoredReportCards()
    const recordsByKey = new Map(
      existingRecords.map((record) => [
        buildReportCardStorageKey(record.studentId, record.className, record.term, record.session),
        record,
      ]),
    )

    marksData.forEach((student) => {
      const storageKey = buildReportCardStorageKey(
      ...
      const existing = recordsByKey.get(storageKey)
      const trimmedRemark = student.teacherRemark ? student.teacherRemark.trim() : ""
      const existingSubjectRemark = existing?.subjects?.find(
        (entry) => entry.name.trim().toLowerCase() === selectedSubject.trim().toLowerCase(),
      )?.remark
      const subjectRecord: ReportCardStorageSubject = {
        name: selectedSubject,
        ca1: Number.isFinite(student.firstCA) ? Math.round(student.firstCA) : 0,
        ca2: Number.isFinite(student.secondCA) ? Math.round(student.secondCA) : 0,
        assignment: Number.isFinite(student.noteAssignment) ? Math.round(student.noteAssignment) : 0,
        exam: Number.isFinite(student.exam) ? Math.round(student.exam) : 0,
        total: Number.isFinite(student.grandTotal) ? Math.round(student.grandTotal) : 0,
        grade: student.grade,
        remark: trimmedRemark.length > 0 ? trimmedRemark : existingSubjectRemark ?? null,
      }

      const updatedRecord: ReportCardStorageRecord = {
        id: existing?.id ?? `draft-${storageKey}`,
        studentId: String(student.studentId),
        studentName: student.studentName,
        className: selectedClass,
        term: termLabel,
        session: selectedSession,
        subjects: mergeSubjectEntries(existing?.subjects ?? [], subjectRecord, selectedSubject),
        classTeacherRemark:
          additionalData.classTeacherRemarks[student.studentId]?.trim() || existing?.classTeacherRemark || null,
        headTeacherRemark: existing?.headTeacherRemark ?? null,
        createdAt: existing?.createdAt ?? now,
        updatedAt: now,
      }

      recordsByKey.set(storageKey, updatedRecord)
    })

    writeStoredReportCards(Array.from(recordsByKey.values()))
  }, [
