...
export function EnhancedReportCard({ data }: { data?: RawReportCardData }) {
  ...
  const preparePdfDocument = useCallback(async () => {
    ...
    const usableWidth = Math.max(pdfWidth - (margins.left + margins.right), 0)
    const usableHeight = Math.max(pdfHeight - (margins.top + margins.bottom), 0)
    ...
    const extraVerticalSpace = Math.max(usableHeight - renderHeight, 0)
    const offsetX = margins.left
    const offsetY = margins.top + extraVerticalSpace / 2
